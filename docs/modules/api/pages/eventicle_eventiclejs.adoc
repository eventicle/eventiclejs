...
s|xref:eventicle_eventiclejs_RedisStreamConfig_interface.adoc[RedisStreamConfig]
|
|===
...
https://github.com/confluentinc/confluent-kafka-javascript/blob/dev\_early\_access\_development\_branch/MIGRATION.md
...
m|config
m|KafkaConfig
|as per @confluentinc/kafka-javascript kafkaJS compat layer
...
========
[id="eventicle_eventiclejs_eventClientOnRedis_function_1"]
=== eventClientOnRedis(config)

========

Factory function to create a Redis Streams-backed EventClient.

The returned client implements the full EventClient interface using Redis Streams for event persistence and delivery. It supports hot, cold, and cold-hot subscriptions with consumer group load balancing.
...
m|config
m|xref:eventicle_eventiclejs_RedisStreamConfig_interface.adoc[RedisStreamConfig]
|Redis connection and stream configuration

s|Returns
m|Promise&lt;xref:eventicle_eventiclejs_EventClient_interface.adoc[EventClient]&gt;
|A connected EventClient wrapped with transactional support
|===
...
========
[id="eventicle_eventiclejs_shutdownAllAdapters_function_1"]
=== shutdownAllAdapters()
